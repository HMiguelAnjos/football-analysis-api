...
class FootballDataProvider(Protocol):
    ...
    def get_red_cards(self, fixture_id: int) -> dict[int, int]:
        """Expulsões por team_id no jogo (modelo ao vivo). {} se n/a."""
        ...

    def get_live_player_shots(self, fixture_id: int) -> list[dict]:
        """Chutes de cada jogador no jogo atual (especialista em chutes a gol).
        Lista de {player_id, name, team_id, minutes, shots_total, shots_on}."""
        ...
